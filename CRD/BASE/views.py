...
from .serializers import (
    ...
    ProducerSerializer,
    CgArtist2Serializer,
    Supervisor2Serializer,
    TaskSerializer2_noId,
)
...
def createdir(request):
    if request.method == "POST":
        req = requests.post(
            f'{path_ngrok}make_production_directories/{request.data.get("a")}',
            json={"a": request.data.get("a")},
        )
...
def createProd(request):
    ...
    if request.method == "POST":
        req = requests.post(
            f"{path_ngrok}make_production_directories/{pathProduction}",
            json={"a": pathProduction},
        )
...
def launch_scene(request):
    ...
    try:
        ...
        response = requests.post(
            f"{path_ngrok}lancer_scene", json={"blend_file_path": blend_file_path}
        )
        ...
        if response.status_code == 200:
            ...
            return Response(
                {"ouverture scene": "failed", "details": response.text},
...
def createTask(request):
    ...
    if serializer.is_valid():
        ...
        print(
            "idDeProd=", idDeProd, "type:", type(idDeProd)
        )  # Ajouter ce type de log pour déboguer
...
def get_task2_ids_by_production(request, production_id):
    ...
    tasks2 = (
        production.production_tasks2.all()
    )  # production_tasks2 est le related_name défini dans la classe Task2
...
# name
@api_view(["PUT"])
def updateTaskName(request, id):
    try:
        instanceTask = Task2.objects.get(id=id)
        serializerTask = TaskSerializer2(instanceTask, data=request.data, partial=True)
...
def updateSup(request, id):
    try:
        ...
        supervisor_name = request.data.get("supervisor2ID")

        if supervisor_name:
            try:
                supervisor_instance = Supervisor2.objects.get(name=supervisor_name)
                request.data["supervisor2ID"] = supervisor_instance.id
            except Supervisor2.DoesNotExist:
                return Response(
                    {
                        "error": f"Superviseur with name '{supervisor_name}' does not exist."
                    },
                    status=400,
                )
        ...
        return Response(serializerTask.errors, status=400)

    except Task2.DoesNotExist:
        return Response({"error": f"Task with id '{id}' does not exist."}, status=404)
    except Exception as e:
        return Response({"error": str(e)}, status=500)
...
def updateArtist(request, id):
    try:
        ...
        cgArtist_name = request.data.get("cgArtist3Id")

        if cgArtist_name:
            try:
                cArtist_instance = CgArtist3.objects.get(name=cgArtist_name)
                request.data["cgArtist3Id"] = cArtist_instance.id
            except CgArtist3.DoesNotExist:
                return Response(
                    {
                        "error": f"SupervcGArtist with name '{cgArtist_name}' does not exist."
                    },
                    status=400,
                )
...
def updateStatus(request, id):
    try:
        instanceTask = Task2.objects.get(id=id)
        if request.data["completed"] == "true" or request.data["completed"] == "false":
            serializerTask = TaskSerializer2(
                instanceTask, data=request.data, partial=True
            )
            if serializerTask.is_valid():
                serializerTask.save()
        else:
        ...
        return Response({"status": request.data["completed"]})
    except Task2.DoesNotExist:
        return Response({"error": f"la tache {id} n'existe pas"}, status=404)
